...
static void mySwap (String & ioLeft, String & ioRight) {
...
//--- Sort using 'quick sort' algorithm
  if (inFirst < inLast) {
    ...
    while (i < j) {
      ...
      if (i < j) {
        mySwap (gFileNameArray [i], gFileNameArray [j]) ;
      }
    }
    mySwap (gFileNameArray [j], gFileNameArray [inFirst]) ;
...
void plotGraph (void) {
  ...
  //--- Compute temperature max
    ...
    tft.setCursor (16, 32) ;
    tft.drawRect (tft.getCursorX (), tft.getCursorY (), w, h, ILI9341_WHITE) ;
    ...
    for (uint8_t i = 1 ; i < gProgramDescriptor.mPointCount ; i++) {
      ...
      tft.drawLine (x0, y0, x1, y1, ILI9341_YELLOW) ;
      x0 = x1 ; y0 = y1 ;
    }
...
static void drawGrid (void) {
  ...
  const uint16_t smallLineColor = ILI9341_NAVY ;
  const uint16_t largeLineColor = ILI9341_LIGHTGREY ;
...
//--- Print values
  tft.setTextSize (2) ;
  const uint32_t displayCount = (gProgramDescriptor.mPointCount < 26) ? gProgramDescriptor.mPointCount : 26 ;
  for (uint32_t i = 0 ; i < displayCount ; i++) {
    tft.setTextColor (ILI9341_YELLOW) ; setCursorForTable (i, false) ; tft.printf("%4u", gProgramDescriptor.mPoints[i].mTime) ;
    tft.setTextColor (ILI9341_PINK) ; setCursorForTable (i, true) ; tft.printf("%4u", gProgramDescriptor.mPoints[i].mTemperatureReference) ;
  }
